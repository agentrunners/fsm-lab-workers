...
async function tailAlertIssue(action) {
  const body = [
    ...
    '',
    `Trigger: ${action.trigger === 'turns' ? `${action.tail_turns} tail turns ≥ N=3` : `riding ≥ X=${action.window_min}min`} — the free tail is serving turns: the paid lanes are dry (a bridge posture, never a home).`,
    `Tail turns: ${action.tail_turns} · since ${action.tail_since ?? '?'}`,
    '',
    'The tail is free-lane-class (on the cc lane, cohere since the s24 flip): turns COMPLETE but generic — a degraded-but-alive fleet, not a paused one (no action forced: the tail burns nothing).',
    '',
    '**Operator actions** (the tail knob is engine-side — today only the cc lane carries one; the codex chain pins no `:free` slot, so this alert is cc-only by construction until another engine grows a tail):',
    '- top up a paid key (OpenRouter credits), or',
    '- swap in a funded key (the `OPENROUTER_API_KEY` / `OPENROUTER_API_KEY_2` repo secrets), or',
    '- set the cc lane\'s tail knob `CC_TAIL_MODEL` to another `:free` model (e.g. `nvidia/nemotron-3.5-lightning:free`, the s24-demoted slug), or',
