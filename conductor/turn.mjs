...
async function tailAlertIssue(action) {
  const body = [
    ...
    '',
    'The tail is free-lane-class (cohere since the s24 flip): turns COMPLETE but generic — a degraded-but-alive fleet, not a paused one (no action forced: the tail burns nothing).',
    '',
    '**Operator actions**:',
    '- top up a paid key (OpenRouter credits), or',
    '- swap in a funded key (the `OPENROUTER_API_KEY` / `OPENROUTER_API_KEY_2` repo secrets), or',
    '- set `CC_TAIL_MODEL` to another `:free` model (e.g. `nvidia/nemotron-3.5-lightning:free`, the s24-demoted slug), or',
