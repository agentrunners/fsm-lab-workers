...
// values HERE so a typo fails at the door, not at genesis. s24/B1: 'codex'
// joins the vocabulary (the OpenAI Codex engine epoch — multi-engine
// design D1); EPOCH-level ONLY — D16 rejects a per-entry mode in the
// tasks form (see the entry loop in validateSpec).
export const INTAKE_MODES = ['mock', 'real', 'cc', 'codex'];
...
// s24/B1 (multi-engine design D16): the ENTRY key set drops `mode` — a
// per-entry mode is REJECTED at the door (it was validated here, then
// silently DROPPED by specToTask — conductor-core.mjs — an epoch runs ONE
// harness, so a per-entry mode is a rejected ambiguity, not dropped
// metadata). `mode` stays in TASK_FIELD_KEYS itself: the single-task top
// level is both task and epoch level (SPEC_FIELD_KEYS inherits it), and
// the entry loop emits the SPECIFIC epoch-level pointer instead of the
// generic unknown-key drip.
const ENTRY_FIELD_KEYS = TASK_FIELD_KEYS.filter(k => k !== 'mode');
  ...
  if (Object.prototype.hasOwnProperty.call(spec, 'tasks')) {
    if (!Array.isArray(spec.tasks)) {
      ...
      spec.tasks.forEach((e, idx) => {
        ...
        // s24/B1 (design D16): per-entry `mode` REJECTS at the door — the
        // mode is EPOCH-level (one harness per epoch); it used to be
        // validated here then silently DROPPED by specToTask, accepting
        // input the machine would never honor. The rejection points the
        // operator at the top-level field.
        if (Object.prototype.hasOwnProperty.call(e, 'mode')) {
          errors.push(`${prefix}mode is epoch-level — set it once at the top of the block (mode: ${INTAKE_MODES.join(' | ')}), not per entry`);
        }
        const r = validateTaskFields(e, { issue, prefix, requireId: true, only: ENTRY_FIELD_KEYS });
...
export function howToComment() {
  return [
    ...
    '- `mode` optional — one of mock | real | cc | codex (epoch-level — set it once at the top of the block; a per-entry mode in the `tasks:` form is rejected)',
