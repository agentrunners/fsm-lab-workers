  ...
  if (Object.prototype.hasOwnProperty.call(spec, 'artifacts')) {
    ...
    if (!Array.isArray(arts)) {
      ...
      if (arts.length > ARTIFACTS_MAX) {
        errors.push(`artifacts carries ${arts.length} paths (cap ${ARTIFACTS_MAX} — the envelope's ENVELOPE_ARTIFACTS_MAX; more would die at every worker gate)`);
      }
...
export function howToComment() {
  return [
    ...
    '- `lease_minutes` optional — integer [3, 120]; `milestone` optional — integer [1, 9]',
