...
function validateTaskFields(t, { issue, prefix = '', requireId = false, only = null } = {}) {
  const errors = [];
...
export function validateSpec(spec, { issue, raw, problems = [] } = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(spec, 'tasks')) {
    if (!Array.isArray(spec.tasks)) {
      ...
      // is a rejection)
      const TOP_KNOWN = ['tasks', 'mode', 'lease_minutes'];
      ...
      // shared rule table (only lease_minutes/mode are checked on the shim)
      const shim = {};
      ...
      errors.push(...validateTaskFields(shim, { issue, only: ['lease_minutes', 'mode'] }).errors);
    }
  ...
  // messages (the pre-B-1 pins hold verbatim)
  const r = validateTaskFields(spec, { issue });
  errors.push(...r.errors);

  // unknown keys: REJECT, listing them (typo protection)
  const KNOWN = ['id', 'title', 'accept', 'behavior', 'deps', 'artifacts', 'lease_minutes', 'milestone', 'mode', 'tasks'];
...
export function howToComment() {
  return [
    ...
    '- `tasks` optional — 1–32 one-line entries; the epoch-level keys beside it are `mode` and `lease_minutes`',
