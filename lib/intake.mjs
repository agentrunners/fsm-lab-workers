...
// ---------------------------------------------------------------------------
// specToTask(spec, { issue, bodySha8 }) — the genesis task (§2e).
//
// makeGenesis (§5, lane A) feeds this to genesis(): one task per issue. The
// accept text rides `spec` into assembleDispatchPayload's prompt (t.spec is
// embedded there as quoted data); behavior 'real' marks an accept-criteria
// task (the mock behaviors come straight from the door's vocabulary).
// deps are ALWAYS [] (F-1 — the door rejected them; the genesis shape keeps
// the field for the W-D future).
// ---------------------------------------------------------------------------
export function specToTask(spec, { issue, bodySha8 } = {}) {
  const s = spec || {};
  const id = typeof s.id === 'string' && ID_RE.test(s.id) ? s.id : `task-i${issue}`;
  const task = {
    id,
    title: s.title || id,
    behavior: s.behavior ?? 'real',   // accept-criteria tasks carry 'real'
    work_ms: 4000,
    deps: [],                          // F-1 (L1-B1): deps CUT until W-D
    spec: { issue },
  };
  if (s.accept !== undefined) task.spec.accept = s.accept;
  if (s.artifacts !== undefined) task.spec.artifacts = s.artifacts;
  return task;
}
