...
//                  T46/W-D lane B (§D4-M3, the v2 fold): OPTIONAL lane telemetry —
//                  lane_stats / key_index / hop_telemetry ride the REPORT record when
//                  the queue line's outcome carries them (additive, F8-safe; rebuild
//                  ignores them; absent on every legacy record — old fixtures apply
//                  unchanged). The journal IS the durable source the ops console's
//                  LANE section reads (transcripts GC in 7 days; the journal stays).
...
// cross-module equality so the two lists cannot drift). s24/B1: 'codex'
// joins (the OpenAI Codex engine epoch — multi-engine design D1; the
// adapter ships in parallel as B2 and the worker routes it lazily).
export const GENESIS_MODES = ['mock', 'real', 'cc', 'codex'];
...
export const CONFIG_BOUNDS = {
  max_parallel: [1, 32],
  // s22/B-1 DECISION (documented): the schema bound STAYS [1,120] — the floor
  // moved only on the UNTRUSTED seams (the intake door's LEASE_MIN=3 and the
  // conductor's carry clamp, both keyed to LEASE_FLOOR_MINUTES in
  // conductor-core). Rationale: (a) this bound governs the TRUSTED operator
  // seam (genesis + `configure`) — the whole lease-TIMEOUT machinery (reap +
  // reassign) is a designed drill feature, and the test suite's fast-drill
  // idiom legitimately configures lease_minutes 1-2 to exercise it quickly;
  // (b) moving it would hard-throw validateConfig on REPLAY of pre-floor
  // journal genesisSpecs and on reset's F12 config ADOPTION from legacy
  // states carrying 1-2 (recovery broken) — the reset path's config
  // validation must keep validating, not start rejecting history. An
  // operator who configures 1-2 on a real/cc chain re-enters the trap
  // knowingly; the door + carry close the accidental path.
  lease_minutes: [1, 120],
  max_attempts: [1, 9],
...
export function genesis({ config, project, chainId, now, mode = 'mock', issue = null }) {
  ...
  return {
    ...
    dedup: [],
    // s23/X27 (the genesis window init — EVIDENCE §X27 quirk 7): genesis
    // must initialize EVERY array/window field the runtime scans — an
    // absent key means "inherits whatever the scan's next write lands",
    // not "clean". The epoch boundary is a window-CLEAR boundary (the SAME
    // semantics as the resume control's clear below): any in-flight reports
    // from the PREDECESSOR epoch are stragglers of the dead epoch —
    // excluded from the fresh epoch's count-trigger by the C-3 straggler
    // gate (conductor-core reads this stamp exactly like the resume stamp).
    // F8/additive: rebuild derives BOTH fields (the reset replay re-runs
    // genesis with the journaled spec.now; the resume replay re-stamps from
    // the journaled ts) — live and replay converge at every boundary.
    budget_window: [],
    budget_window_cleared_at: now,
  };
}
...
export function applyEvent(state, ev, now) {
  ...
  switch (ev.kind) {
    case 'TICK': {
      ...
      // T46/W-C3 (X24) carve-out: the ONE exception is the pinger's wake —
      // conductorTick mints an audit-only liveness MARKER for it (see
      // lib/conductor-core.mjs) so the pinger's heartbeat leaves a journal
      // trail exactly when the chain is idle. Nothing changes HERE: the
      // marker is minted at the conductorTick layer, applyEvent stays
      // byte-identical for every wake.
      ...
      // T46/W-C3 (X24): the TICK record carries the MINTED event id
      // (`tick-<reason>-<ms>`) as an ADDITIVE field — the journal becomes the
      // tick's provenance trail (rebuild ignores it: the replay below reads
      // seq/actor/ts only, so the field is additive-F8-safe; legacy records
      // without it replay byte-identically). For the pinger's dispatch this
      // is the `tick-pinger-<ms>` marker — X24's journal criterion.
      return { state: s, applied: true, reason: 'tick', eventOut: jrec(s, { kind: 'TICK', seq: s.chain.seq, actor: ev.actor || 'chain', event_id: ev.event_id ?? null }, now) };
    }
    case 'REPORT': {
      ...
      // T46/W-D lane B (§D4-M3): the OPTIONAL lane telemetry carry — every
      // outcome class funnels the same fields into the journal RECORD (the
      // durable console source) and the task's last_result projection.
      // Absent on legacy queue lines: the object is EMPTY and every
      // transition below is byte-identical to the pre-W-D shape (the
      // old-fixture compatibility requirement is HARD).
      const lane = laneOutcomeFields(outcome);
      if (outcome.status === 'progress') {
        return transition(s, t, 'in_progress', now, 'worker-progress', ev,
          { kind: 'REPORT', task: t.id, lease: ev.lease, to: 'in_progress', run_id: ev.run_id, ...lane },
          { last_result: { status: 'progress', run_id: ev.run_id, ...lane } });
      }
      if (outcome.status === 'done') {
        return transition(s, t, 'done', now, 'worker-done', ev,
          { kind: 'REPORT', task: t.id, lease: ev.lease, to: 'done', run_id: ev.run_id, ...lane },
          { last_result: { status: 'done', run_id: ev.run_id, artifact: outcome.artifact || null, duration_ms: outcome.duration_ms || null, ...lane } });
      }
      if (outcome.status === 'failed' || outcome.status === 'work_failed') {
        ...
        return transition(s, t, dest, now, `worker-failed(attempts=${t.attempts})`, ev,
          { kind: 'REPORT', task: t.id, lease: ev.lease, to: dest, run_id: ev.run_id, ...lane },
          { last_result: { status: outcome.status, run_id: ev.run_id, error: outcome.error != null ? String(outcome.error).slice(0, 200) : null, ...lane } });
      }
      if (outcome.status === 'deadline') {
        ...
        return transition(s, t, dest, now, `worker-deadline(attempts=${t.attempts})`, ev,
          { kind: 'REPORT', task: t.id, lease: ev.lease, to: dest, reason: 'deadline', run_id: ev.run_id, error, ...lane },
          { lease: null, last_result: { status: 'deadline', run_id: ev.run_id, error, ...lane } });
      }
      if (outcome.status === 'poison') {
        ...
        return transition(s, t, 'quarantined', now, 'worker-poison', ev,
          { kind: 'REPORT', task: t.id, lease: ev.lease, to: 'quarantined', reason: 'poison', run_id: ev.run_id, error, ...lane },
          { last_result: { status: 'poison', run_id: ev.run_id, error, ...lane } });
      }
      if (outcome.status === 'infra_failed') {
        ...
        if (infraAttempts >= INFRA_RETRY_MAX) {
          return transition(s, t, 'quarantined', now, `infra-exhausted(infra_attempts=${infraAttempts})`, ev,
            { kind: 'REPORT', task: t.id, lease: ev.lease, to: 'quarantined', reason: 'infra-exhausted', run_id: ev.run_id, error, ...lane },
            { infra_attempts: infraAttempts, last_result: { status: 'infra_failed', run_id: ev.run_id, error, ...lane } });
        }
        ...
        return transition(s, t, 'ready', now, `infra-retry(lane-unavailable)`, ev,
          { kind: 'REPORT', task: t.id, lease: ev.lease, to: 'ready', reason: 'infra-retry', run_id: ev.run_id, error, ...lane },
          { attempts: Math.max(0, t.attempts - 1), lease: null, infra_attempts: infraAttempts, last_result: { status: 'infra_failed', run_id: ev.run_id, error, ...lane } });
      }
    ...
    case 'CONTROL': {
      ...
      if (cmd === 'pause') {
        ...
        s.budget_window = [];
        // s21/C-3 (audit a1, live-observed 3x): stamp the clear — the
        // straggler-race gate. Leases issued BEFORE this stamp belong to
        // the paused cohort; their post-resume quota reports are stragglers
        // of the OLD storm (a ~9-min grind racing the resume), not a new
        // one. The trigger excludes straggler-marked entries (the race
        // produced X23's re-pause + the s20 close's 4th).
        s.budget_window_cleared_at = now;
      }
      ...
      // T46/W-C1 (F-7): the pause PAYLOAD rides the record too — rebuild
      // derives paused_reason from it (the projection rule). Additive,
      // rebuild-safe: absent on legacy/plain pauses.
      const cnote = ev.note != null && String(ev.note) !== '' ? String(ev.note).slice(0, 200) : null;
      return { state: s, applied: true, reason: `control:${cmd}`, eventOut: jrec(s, { kind: 'CONTROL', command: cmd, actor: ev.actor ?? null, note: cnote, ...(ev.payload && typeof ev.payload === 'object' ? { payload: ev.payload } : {}) }, now) };
    }
    default:
...
function reject(s, ev, now, reason) {
  ...
  // s21/O-4 (audit a5): a rejected CONTROL names its COMMAND — the audit
  // trail (and the conductor's CONTROL-REJECTED alert arm) can say WHICH
  // operator knob failed, not just that "a control" did. Additive field,
  // rebuild-safe (applied:false records never replay).
  if (ev.kind === 'CONTROL' && ev.command) out.command = ev.command;
...
// T46/W-D lane B (§D4-M3): the REPORT line's OPTIONAL lane telemetry —
// lane_stats (the cc bridge-JSONL aggregate), key_index (D3's pool pick),
// pool_size (the pick's modulo base — rides EXACTLY like key_index; the
// W-D review fold A4 / lens-2 F1: the journaled key_index is an index into
// the pool ARRAY, so without its base the historical slots go ambiguous
// across pool redeploys), hop_telemetry (the real lane's per-hop). Guards
// only, NO slicing (the aggregate is the console's source — corrupting it
// here would poison every downstream view); EMPTY object on legacy lines so
// the spread is a no-op (byte-identical transitions — the compat
// requirement).
// s24/B9: mode — the turn's ENGINE provenance (composeReportOutcome's
// envelope.mode carry). The journal's REPORT records carried NO engine
// marker (lane_stats' model slugs are shared between the cc and codex
// chains), so the console's engine column had nothing to render from; the
// mode rides the SAME guard family — absent on legacy/pre-B9 outcomes (the
// spread stays a no-op, old-fixture compat holds), journaled verbatim on
// every new turn (validated against the 4-member vocabulary upstream at
// envelopeFromDispatch).
function laneOutcomeFields(outcome) {
  const f = {};
  if (outcome && outcome.lane_stats && typeof outcome.lane_stats === 'object' && !Array.isArray(outcome.lane_stats)) {
    f.lane_stats = outcome.lane_stats;
  }
  if (outcome && Number.isFinite(outcome.key_index)) f.key_index = outcome.key_index;
  if (outcome && Number.isFinite(outcome.pool_size)) f.pool_size = outcome.pool_size;
  if (outcome && Array.isArray(outcome.hop_telemetry) && outcome.hop_telemetry.length) f.hop_telemetry = outcome.hop_telemetry;
  if (outcome && typeof outcome.mode === 'string' && outcome.mode !== '') f.mode = outcome.mode;
  // s25-r1/R3: the degraded-transcript count rides the journal RECORD (the
  // durable surface the run-log note never had); absent = the legacy shape
  if (outcome && Number.isFinite(outcome.transcript_degraded)) f.transcript_degraded = outcome.transcript_degraded;
  return f;
}
...
export function clock(state, now, nextMilestone, opts = {}) {
  ...
  let passAssigns = 0;   // F-10: this pass's spend (the fn adds the tick-wide count)
  for (const t of ready) {
    ...
    if (dispatchBudgetFn) {
      const budgetNow = Math.max(0, Math.floor(dispatchBudgetFn(passAssigns)));
      if (budgetNow <= 0) {
  ...
  // T46/W-C1-R (lens-1 BLOCKING-1): the MILESTONES_TOTAL BOUND — the pass
  // consults the injected generator ONLY while milestone < milestones_total.
  // The live adapter wires the MOCK drill's generator unconditionally; an
  // intake epoch (single task, milestones_total=1) completed its task and
  // SPROUTED the mock M2+M3 (~10 phantom dispatches per spec, the rollover
  // never fired — the wave's centerpiece dead on the live path). The bound
  // is outcome-neutral for mock epochs (the generator already nulls past
  // its last milestone) and makes milestones_total load-bearing (it was
  // written at genesis and read NOWHERE — the lens's finding).
  const tasksArr = Object.values(s.tasks);
  const allTerminal = tasksArr.length > 0 && tasksArr.every(t => TERMINAL.has(t.status));
  const maxMilestones = Number.isFinite(s.project.milestones_total) ? s.project.milestones_total : Infinity;
  if (allTerminal && s.project.phase !== 'done') {
    // the generator is consulted ONLY within the milestones_total bound;
    // out of bounds falls through to phase-done (the else below) — never a
    // hung executing phase
    const next = s.project.milestone < maxMilestones && nextMilestone ? nextMilestone(s.project.milestone) : null;
    if (next && next.tasks && next.tasks.length) {
      ...
      s.chain.halted = true;
      bumpVersion(s);
      // T46/W-C2-R (F7-adjacent): the record carries the COMPLETING epoch's
      // stats/tasks/issue — the m-3 comment reads THEM, never the live state
      // (a same-tick intake rollover replaces the state with the NEXT
      // epoch; the old comment shape posted the new epoch's zeroed stats
      // under "Epoch COMPLETE" — done 0/1 — the lens-1 probe).
      J({
        kind: 'PHASE', from: fromPhase, to: 'done',
        ...(degraded ? { degraded: true } : {}),
        stats: { ...s.stats, taskCount: tasksArr.length },
        taskIds: tasksArr.map(t => t.id),
        ...(s.project?.issue != null ? { issue: s.project.issue } : {}),
      });
...
export function rebuild(genesisState, journalRecords, { nextMilestone } = {}) {
  ...
  for (const j of replay) {
    ...
    // T46/W-C3 (X24): the counter narrows to REJECTED-kind records ONLY —
    // the pinger's held-chain liveness MARKER is also applied:false
    // (audit-only) but is NOT a rejection; counting it here would diverge
    // the rebuild projection from live (the live side never counts it — it
    // mints no REJECTED record and bumps no counter). Every applied:false
    // record before W-C3 is kind REJECTED, so the narrowing is
    // byte-identical for all existing classes.
    if (j.applied === false) {
      if (j.kind === 'REJECTED') s.stats.rejected_events += 1;
    ...
    switch (j.kind) {
      ...
      case 'REPORT': {
        ...
        // T46/W-C2-R (F1/L2-9): the PR-stamp class — a REPORT record carrying
        // `pr` and NO lease (lib/task-pr.mjs stampPr mints it pointer-only).
        // The lease-gated transition below can never replay it (a done task's
        // lease is null), so without this branch task.pr is LOST on any
        // journal-replay reconstruction → duplicate PRs / 422 alert livelock.
        // Replay: additive, idempotent (first stamp wins — a twin never
        // overwrites), status/lease untouched.
        if (j.pr != null && j.lease == null && t && t.status === 'done') {
          if (t.pr == null) {
            t.pr = j.pr;
            t.history.push({ at: ts, status: t.status, why: 'replay:PR-STAMP' });
          }
          break;
        }
      ...
      case 'CONTROL': {
        ...
        if (j.command === 'reset' && j.genesisSpec) {
          ...
          s.budget_window_cleared_at = ts;  // s21/C-3: the straggler-gate stamp (replay-consistent)
