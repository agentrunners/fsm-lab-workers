...
// hop_telemetry (the real lane's per-hop). Guards only, NO slicing (the
// aggregate is the console's source — corrupting it here would poison every
// downstream view); EMPTY object on legacy lines so the spread is a no-op
// (byte-identical transitions — the compat requirement).
