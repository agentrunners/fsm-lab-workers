...
// X21 live finding: the floor DEFAULT is OFF (0) until the dispatchBudget
// architecture lands (W-C). The skip-left-assigned bug: a paced dispatch's
// task stays ASSIGNED (the clock already assigned it) — no later tick
// re-attempts it, only the 12-min law-4 flip recovers it, burning infra
// budget toward quarantine. The floor was starving 3 of 4 dispatches every
// long-lease turn. Opt-in via PACING_FLOOR_S env only with the budget fix.
export const PACING_FLOOR_DEFAULT_S = 0;

// M-3/B (46-R2): the ADAPTER's floor default consumes the exported constant
// above (env override wins). While conductor/turn.mjs hardcoded `|| '0'` the
// constant was dead code and reverting the hot-fix-2 default (0 → 300,
// re-introducing the skip-left-assigned bug) left every gate green (lens B
// mutation #2). Unparseable or empty env → the default (fail-open: a bogus
// PACING_FLOOR_S cannot starve dispatches).
export function resolvePacingFloorS(env = process.env) {
  const raw = env.PACING_FLOOR_S;
  const v = parseInt(raw != null && raw !== '' ? raw : String(PACING_FLOOR_DEFAULT_S), 10);
  return Number.isFinite(v) ? v : PACING_FLOOR_DEFAULT_S;
}
