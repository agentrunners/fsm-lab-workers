...
import { mintEventId, PINGER_REASON } from './event-ingest.mjs';   // T46/W-C2 (F-12): the console lane's drain mint — MINT_TABLE is the ONE id-mint source; W-C3: PINGER_REASON for the X24 marker
...
// s22/B-1 (BLOCKING, lens-1): the LEASE FLOOR. The dispatch envelope's
// deadline is min(lease,48)·60s − W2_ENVELOPE_MARGIN_MS — a lease_minutes ≤ 2
// lands that deadline AT OR BEFORE the assign instant (2min − 2min = 0), the
// worker's law-1 start-gate rejects every start as 'late-start', the epoch
// loops re-dispatch → degraded halt with the user's work destroyed — the trap
// lens-1 verified live in the e2e recovery drill. The floor 3 keeps ≥60s of
// post-margin runway. ONE definition, three enforcement seams: the door
// (lib/intake.mjs LEASE_MIN re-exports this), the carry below
// (specLeaseMinutes clamps to it), the fsm schema (CONFIG_BOUNDS stays
// [1,120] — see its comment).
export const LEASE_FLOOR_MINUTES = 3;
...
// T46/s21 C-1 (audit a1, BLOCKING): the scan's UNION planner — law-4 must
// verify a run that materialized in EITHER worker bucket. A repo2-only
// dispatch (the overflow lane's saturated-ladder fallback) has its ONLY run
// in WORKER_REPO_2's worker.yml; a main-only scan can never see the key ->
// the 720s flip to infra_failed 'dispatch-unverified' -> net-zero requeue +
// re-dispatch while the mirror worker still grinds -> x3 INFRA_RETRY_MAX ->
// quarantined 'infra-exhausted' with the work LIVE in bucket 2 (the lane
// that exists to ADD capacity destroying its own tasks). The PAT is
// REQUIRED for the second fetch: the ephemeral job token is same-repo
// scoped (X1a) and the PAT already holds actions:read (it performs the
// cross-repo dispatch itself). Pure: [main] or [main, repo2].
export function verifyScanRepoList(repo, repo2 = null, pat = null) {
  const repos = [repo];
  if (repo2 && pat) repos.push(repo2);
  return repos;
}
...
// s23/B9 (design §2.6): the FREE-TAIL RIDING alert thresholds — pinned
// constants for v1 (the orchestrator's adjudication #4; spec-config knobs
// are the future posture if the live tail drill wants them retuned). N=3
// turns matches the F-6 budget-window threshold family (3 = a real posture,
// not a transient drain edge); X=15min ≈ 5-45 tail turns at the demoted
// nemotron class's 20-160s cadence — ~90-225 at the s24 cohere tail's
// 4-9.5s — well past "a bridge", short of the budget
// pause's epoch-level posture. Whichever fires FIRST.
export const TAIL_ALERT_TURNS = 3;
export const TAIL_ALERT_WINDOW_MS = 15 * 60_000;

// s23/B9: the slug derivation (the adjudicated v1 — no fsm.mjs schema
// change): a REPORT was SERVED BY THE TAIL iff its outcome's lane_stats
// models map carries a `:free` slug with ok>0 — at most ONE lane family
// answers per turn (an answering lane terminates the turn), so free-ok ⟺
// the ANSWERING lane was the chain's `:free` slot. The complement (any
// non-`:free` slug with ok>0) ⟺ a paid lane served — a key recovered.
// Pure + exported: the drain's counter, the alert trigger, and the tests
// all key on the SAME derivation (the dispatchBudgetFromWall discipline —
// pin the arithmetic where it lives).
export function tailServedFromOutcome(outcome) {
  if (!outcome || typeof outcome !== 'object' || Array.isArray(outcome)) return { tail: false, paid: false };
  const ls = outcome.lane_stats;
  if (!ls || typeof ls !== 'object' || Array.isArray(ls)) return { tail: false, paid: false };
  const models = ls.models;
  if (!models || typeof models !== 'object' || Array.isArray(models)) return { tail: false, paid: false };
  let tail = false;
  let paid = false;
  for (const [slug, mm] of Object.entries(models)) {
    if (!mm || typeof mm !== 'object') continue;
    if (Number(mm.ok) > 0) {
      if (typeof slug === 'string' && slug.endsWith(':free')) tail = true;
      else paid = true;
    }
  }
  return { tail, paid };
...
// s25/A2-M1 (the codex arm, same F-6 on the second engine): the codex quota
// vocabulary joins the match — the bare lane detail `codex-error_rate_limit`
// / `codex-error_quota_daily` (codexLaneOutcome's rotate detail,
// worker/codex-adapter.mjs:378) and the terminus that embeds them,
// `lane-exhausted(n/m lanes, last codex-error_rate_limit)` (:867) — until
// this line a codex quota wall burned every affected task through 3 full
// infra-retry re-turns into infra-exhausted quarantine with NO budget-pause
// and NO alert-first (the multi-engine-design Q3/D15 spend guard claimed but
// never wired on codex). The EXCLUSIONS are deliberate: codex-error_credits
// and codex-error_auth are DEAD-KEY classes — the W-D lens-1 F2 narrowing
// was CORRECT for them (a 401/402-quarantined key's remedy is "swap the
// pool secret", not "wait out the quota wall"; the key-class JUMP + the
// turn-side rotation already recover a live key within the turn);
// error_network / error_model_400 / error_in_flight_budget are upstream /
// config / transient — never the quota WAIT the pause exists for. Only the
// two rate/quota classes arm F-6, in BOTH the bare and the terminus form.
export function isQuotaDetail(detail) {
  const s = String(detail ?? '');
  return s.includes('lane-429')
    || /error-as-answer\(rate limit\)/i.test(s)
    || s.includes('codex-error_rate_limit')
    || s.includes('codex-error_quota_daily');
}
...
// s21/A4-F1 (audit a4, MAJOR): the spec's lease_minutes — door-validated
// and advertised verbatim in howToComment — is carried into the epoch's
// genesis config at the rollover and at reset {from_queue}. Previously
// validated-and-advertised but silently DROPPED: a mode:cc task with a
// 9-min grind on a chain configured with a short lease got lease-reaped
// mid-work, re-dispatched (duplicate spend), its late report orphaned
// 'stale-lease', and after 3 attempts the epoch ended DEGRADED — the
// user's completable work lost through a knob they were told works.
// s22/B-1 (BLOCKING, lens-1): the CARRIED value is bounded to
// [LEASE_FLOOR_MINUTES(3), 120]. The door now rejects lease_minutes < 3 at
// intake (the advertised bounds moved); the carry CLAMPS defensively — a
// pre-floor queue line (written by the old door) or a hand-crafted one
// still lands a sane lease, never the work-destroying 1-2min trap (the
// envelope deadline = min(lease,48) − 120s must be > 0 at assign). PURE:
// returns the clamped int when the spec carries an in-[1,120] value (the
// door's YAML-ish parse keeps scalars as STRINGS — '45' — so parseInt
// here), else null (the chain's current config stands; out-of-bounds
// garbage is defense-in-depth — the door already rejects it).
export function specLeaseMinutes(spec) {
  const n = parseInt(spec?.lease_minutes, 10);
  if (!Number.isInteger(n) || n < 1 || n > 120) return null;
  return Math.max(n, LEASE_FLOOR_MINUTES);
}

// s22/B-1 (the s22/Q1 adjudication): the spec-level CAPACITY knobs. The
// staged design's Q1 exposed the dependency: with the repo var
// WORKER_OVERFLOW_AT at its production posture (12-15) a max_parallel:4
// drill never overflows (the pre-flight arm needs in-flight ≥ the
// threshold), and the X27 capacity soak needs its OWN posture per epoch.
// The knob pair is therefore spec-level: `max_parallel` [1,32] (the
// machine's CONFIG_BOUNDS ceiling — the door caps tasks at 32 to match) and
// `overflow_at` [1,32] (the pre-flight threshold), carried into the genesis
// config at the rollover AND reset {from_queue} — the EXACT A4-F1
// specLeaseMinutes pattern: parseInt the string scalars (the door's YAML-ish
// parse keeps them as strings), bounds-check, else null (absent/garbage →
// the knob is ABSENT — the chain's current config / the env lane stands; the
// door already rejects out-of-bounds values at intake, so out-of-bounds here
// is defense-in-depth, and null is the safe fallback: fail to the standing
// posture, never to a wide epoch). PURE.
export function specCapacity(spec) {
  const mp = parseInt(spec?.max_parallel, 10);
  const oa = parseInt(spec?.overflow_at, 10);
  return {
    max_parallel: Number.isInteger(mp) && mp >= 1 && mp <= 32 ? mp : null,
    overflow_at: Number.isInteger(oa) && oa >= 1 && oa <= 32 ? oa : null,
  };
}

// s22/B-1 (the overflow threshold's precedence — the brief's exact chain):
//   state.config.overflow_at ?? env.WORKER_OVERFLOW_AT ?? DEFAULT
// The STATE view wins (a drill epoch runs its own capacity posture while the
// repo var stays production — Q1's resolved shape: keep the var wherever it
// is, the spec carries its own posture per epoch); an absent/null config
// value falls to the env-parsed lane value (which itself folds
// WORKER_OVERFLOW_AT_DEFAULT — the env parse in conductor/turn.mjs is
// `parseInt(env) || DEFAULT`). PURE — extracted so the precedence is
// pinnable against the REAL decision code (the chainContinuationDecision
// discipline), not a source-string check.
export function overflowThreshold(config, envAt = WORKER_OVERFLOW_AT_DEFAULT) {
  return config?.overflow_at ?? envAt;
}
...
// s22/B-1 (staged-mode §2.2/§8 B-1): the spec → epoch-PROJECT mapping — the
// pure seam makeGenesis (conductor/turn.mjs) consumes. A spec carrying a
// `tasks` array (the door's multi-task form) maps EVERY entry through the
// canonical specToTask; a plain spec maps its single task. One mapper for
// both lanes — the door validates each entry with the same per-task rules,
// the genesis maps each entry with the same per-task mapper, and the two
// planes can never disagree.
//
// DEVIATION from staged §8 B-1's formula (deliberate, pinned): B-1 wrote
// `milestones_total = tasks.length` — under the CURRENT machine semantics
// that REINTRODUCES the lens-1 live-sprout bug the bound was built to kill:
// pass 5 consults the generator whenever `milestone(1) < milestones_total`,
// so a 4-task spec epoch with milestones_total=4 sprouts the MOCK M2+M3
// (verified: T-201..T-301, ~10 phantom dispatches) exactly like the
// pre-fold single-task epochs did. The design's own asserts forbid it (§3.2
// A2: "all tasks terminal — done×3 + quarantined×1", the four staged tasks
// ONLY) and its wall budget prices no mock rounds (§2.3). The brief's own
// invariant is the operative clause: "the clock's milestone generator NEVER
// fires for spec epochs — each task IS its own milestone slot" — which the
// milestones_total=1 bound delivers with ZERO new machine surface ("the
// task SET is the project", the single-task comment generalized). The
// per-entry `milestone` key stays door-validated metadata for the W-D
// milestone-aware future (inert today, exactly as pre-B-1).
export function specEpoch(spec, { issue, bodySha8 } = {}) {
  const sp = spec || {};
  if (Array.isArray(sp.tasks)) {
    return { tasks: sp.tasks.map(t => specToTask(t, { issue, bodySha8 })), milestones: 1 };
  }
  return { tasks: [specToTask(sp, { issue, bodySha8 })], milestones: 1 };
}
...
export function conductorTick({
  ...
  for (const c of ctl) {
    if (c.cmd === 'reset') {
      ...
      // s21/A4-F1: the head spec's door-validated lease_minutes rides the
      // genesis config (the from_queue lane gets the same knob the rollover
      // lane gets). s22/B-1: specLeaseMinutes CLAMPS to the floor 3 — the
      // deadline = min(lease,48) − 120s must be > 0 at assign (a carried 1-2
      // is the verified-live work-destroying trap)
      const headLease = specLeaseMinutes(headLine?.spec);
      if (headLease != null) cfg.lease_minutes = headLease;
      // s22/B-1 (the s22/Q1 adjudication — spec-level capacity knobs): the
      // head spec's max_parallel/overflow_at ride the genesis config at the
      // SAME two carry sites (the exact A4-F1 pattern). overflow_at is a NEW
      // config member — genesis's validateConfig only checks its OWN
      // CONFIG_BOUNDS keys, so the extra key rides harmlessly (F12 adoption
      // carries it across plain resets; the overflow PRE-FLIGHT consumes it
      // via the state.config precedence)
      const headCap = specCapacity(headLine?.spec);
      if (headCap.max_parallel != null) cfg.max_parallel = headCap.max_parallel;
      if (headCap.overflow_at != null) cfg.overflow_at = headCap.overflow_at;
      const g = headLine
  ...
  for (const q of queue) {
    // s21/C-3: capture the reporting task's lease issue time BEFORE apply
    // (apply clears t.lease on report) — the straggler-cohort signal.
    const preTask = s.tasks[q.task];
    const preLeaseIssued = preTask && preTask.lease && preTask.lease.issued_at ? preTask.lease.issued_at : null;
    ...
    drained++;
    // s23/X27 (REJECTED-never-arms): a report whose apply() produced a
    // REJECTED journal record (unknown-task / duplicate — the dead epoch's
    // straggler traffic) must NOT arm the budget window or the backstop.
    // Pre-s23 the push below was gated ONLY on the quota shape: the wiped
    // epoch's mirror workers' quota-shaped infra reports landed REJECTED
    // on the fresh epoch but STILL armed the window → the immediate false
    // re-pause 3s after genesis (EVIDENCE §X27 quirk 7). The straggler
    // arithmetic still flows through (the entry shape below is unchanged);
    // both guards stay consistent on the SAME rejection signal.
    const wasRejected = rr.journal.some(j => j.kind === 'REJECTED');
    // s21/C-3: an entry whose lease predates the last resume/window-clear
    // is a STRAGGLER of the pre-pause cohort (a ~9-min grind racing the
    // resume) — journaled for operator audit but EXCLUDED from the
    // count-trigger (the false re-pause class, live-observed 3x).
    // s23/X27: the genesis init extends the SAME gate to the epoch
    // boundary (budget_window_cleared_at === the genesis now).
    const clearedAt = s.budget_window_cleared_at || null;
    const isStraggler = Boolean(clearedAt && preLeaseIssued && Date.parse(preLeaseIssued) < Date.parse(clearedAt));
    // F-6 window tracking (the detail rides the queue line's outcome)
    const oc = q.outcome || {};
    const detail = oc.error ?? oc.detail ?? null;
    if (!wasRejected && oc.status === 'infra_failed' && isQuotaDetail(detail)) {
      if (!Array.isArray(s.budget_window)) s.budget_window = [];
      s.budget_window.push({ ts: now(), task: q.task, detail: String(detail).slice(0, 120), ...(isStraggler ? { straggler: true } : {}) });
      budgetWindowDirty = true;
    }
    // the backstop: an infra-exhausted quarantine whose detail is quota
    // (s21/C-3: a straggler's terminal ladder-burn reflects the PRE-resume
    // lane state — stale information — so it does not arm the backstop;
    // s23/X27: a REJECTED report never arms it either — the exRec find
    // cannot match a rejected apply anyway, the guard keeps both arms
    // consistent on the same signal)
    const exRec = rr.journal.find(j => j.kind === 'REPORT' && j.to === 'quarantined' && j.reason === 'infra-exhausted');
    if (exRec && isQuotaDetail(exRec.error) && !isStraggler && !wasRejected) infraExhaustedQuota = true;
    // s23/B9 — the FREE-TAIL counter (live window state, budget_window's
    // family): a tail-SERVED report (lane_stats.models carries a `:free`
    // slug with ok>0 — tailServedFromOutcome) increments stats.tail_turns
    // and arms stats.tail_since on the FIRST tail turn of the episode; any
    // paid-SERVED report (a paid model ok>0 — a key recovered) RESETS both.
    // REJECTED reports never touch it (the s23/X27 discipline — the dead
    // epoch's straggler traffic is inert on every counter). Mock/real-lane
    // reports carry no lane_stats.models map → inert (the derivation keys
    // on the CC bridge's own telemetry). Rebuild resets it (live-window
    // memory, like budget_window's contents — the next tail report re-arms).
    if (!wasRejected) {
      const served = tailServedFromOutcome(oc);
      if (served.tail) {
        if (!(Number(s.stats.tail_turns) > 0)) s.stats.tail_since = now();
        s.stats.tail_turns = (Number(s.stats.tail_turns) || 0) + 1;
      } else if (served.paid) {
        s.stats.tail_turns = 0;
        s.stats.tail_since = null;
      }
    }
  }
  ...
  if ((Array.isArray(s.budget_window) && s.budget_window.length) || infraExhaustedQuota) {
    ...
    const distinctTasks = new Set((s.budget_window || []).filter(e => !e.straggler).map(e => e.task));
  ...
  // s23/B9: the FREE-TAIL RIDING alert TRIGGER — ALERT-ONLY, no pause (the
  // tail is SERVING work: the budget pause exists to stop attempt burn, and
  // the tail burns nothing — parking the epoch would convert a
  // degraded-but-alive fleet into a stopped one). N=3 tail turns / X=15min
  // of continuous riding, whichever FIRST (TAIL_ALERT_* above). The action
  // re-mints on EVERY riding tick — the adapter's ISSUE-side 24h marker
  // dedup bounds the posts (a failed post leaves no marker → the next tick
  // re-posts: the correlated-failure self-heal, no state-side latch to burn);
  // the counter itself RESETS the moment any paid-served report lands (a key
  // recovered — the drain above). Paused/halted/done chains stay quiet (the
  // F-9 quiesced-noop contract).
  {
    const tailTurns = Number(s.stats.tail_turns) || 0;
    if (tailTurns > 0 && !s.chain.paused && !s.chain.halted && s.project.phase !== 'done') {
      const nowMs = Date.parse(now());
      const sinceMs = typeof s.stats.tail_since === 'string' ? Date.parse(s.stats.tail_since) : NaN;
      const nTrigger = tailTurns >= TAIL_ALERT_TURNS;
      const xTrigger = Number.isFinite(sinceMs) && Number.isFinite(nowMs)
        && nowMs - sinceMs >= TAIL_ALERT_WINDOW_MS;
      if (nTrigger || xTrigger) {
        actionsAll.push({
          type: 'TAIL_RIDING_ALERT',
          tail_turns: tailTurns,
          tail_since: s.stats.tail_since ?? null,
          trigger: nTrigger ? 'turns' : 'duration',
          window_min: Math.round(TAIL_ALERT_WINDOW_MS / 60_000),
        });
        console.log(`TAIL-RIDING-TRIGGER ${nTrigger ? `tail_turns ${tailTurns}>=${TAIL_ALERT_TURNS}` : `riding ${Number.isFinite(sinceMs) && Number.isFinite(nowMs) ? Math.round((nowMs - sinceMs) / 60_000) : '?'}min>=${Math.round(TAIL_ALERT_WINDOW_MS / 60_000)}min`} — ALERT-ONLY (no pause: the free tail is serving turns; the adapter's 24h marker dedup bounds the posts)`);
      }
    }
  }
  ...
  if (intakeQueue.length && s.project.phase === 'done' && !s.chain.paused) {
    ...
    const cfg = { ...s.config, tick_min_interval_s: Math.max(s.config.tick_min_interval_s ?? 0, 25) };
    // s21/A4-F1: the spec's lease_minutes — validated at the door,
    // advertised in the how-to comment — rides the fresh epoch's config (was
    // silently dropped: short-lease chain + long-grind cc task = reaped
    // mid-work, duplicate spend, stale-lease orphans, DEGRADED halt).
    // s22/B-1: the carry CLAMPS to the floor 3 (the door now rejects < 3;
    // this bounds pre-floor queue lines too — deadline = min(lease,48) − 120s
    // must stay > 0 at assign)
    const headLease = specLeaseMinutes(head.spec);
    if (headLease != null) cfg.lease_minutes = headLease;
    // s22/B-1 (the s22/Q1 adjudication — spec-level capacity knobs): the
    // ROLLOVER carry site — max_parallel/overflow_at ride the fresh epoch's
    // config through the same specCapacity parse (the reset from_queue lane
    // above carries the identical knobs; the door validates [1,32] on both)
    const headCap = specCapacity(head.spec);
    if (headCap.max_parallel != null) cfg.max_parallel = headCap.max_parallel;
    if (headCap.overflow_at != null) cfg.overflow_at = headCap.overflow_at;
  ...
  // T46/W-C3 (X24 — the journal half): the pinger's LIVENESS MARKER.
  // A pinger wake on a HELD chain (halted/paused — the common case between
  // epochs) applies NOTHING (F2 above): no record, no seq, no commit — the
  // pinger's heartbeat left NO provenance trail exactly when the chain was
  // idle. X24's journal half fixes that: mint ONE audit-only record
  // (applied:false — no seq bump, no state change, rebuild skips it; the
  // F8-safe audit-only class the REJECTED records already ride) carrying
  // the minted event_id `tick-pinger-<ms>`. The journal is the durable
  // record (state is compacted) — the watch-the-watcher duty scans THIS
  // marker. Shape:
  //   {id:'e<n>', ts, applied:false, kind:'TICK', actor:'pinger',
  //    event_id:'tick-pinger-<ms>', note:'pinger-liveness ...'}
  // Non-pinger held wakes (backstop/manual/watchdog-reprime) stay silent —
  // the noop gate below still quiesces them (the F2 contract unchanged).
  // A DUPLICATE pinger wake (dedup-consumed) mints nothing — the first
  // arrival already journaled it (wakeReason 'duplicate' excluded below).
  // The record lands in `journals`, so the noop gate sees 1 record and the
  // tick COMMITS journal-only — the adapter's chain-continuation decision
  // (chainContinuationDecision, below) keys on the held state and does NOT
  // self-dispatch: the chain stays quiesced, the marker is the only delta.
  if (ev.kind === 'TICK' && ev.actor === PINGER_REASON
      && !wakeApplied && (wakeReason === 'held-paused' || wakeReason === 'held-halted')) {
    mkJ(s, {
      kind: 'TICK', actor: PINGER_REASON,
      event_id: ev.event_id ?? null,
      note: 'pinger-liveness (held chain — audit-only marker, no state change, no self-dispatch)',
    }, false);
    console.log(`PINGER-MARKER journaled ${ev.event_id} (chain ${s.chain.paused ? 'paused' : 'halted'} — audit-only liveness datum; state untouched, no self-dispatch)`);
  }
...
// ---------------------------------------------------------------------------
// T46/W-C3 (X24): the chain-continuation decision — extracted from
// conductor/turn.mjs's self-dispatch gate so the no-self-dispatch-on-held
// contract is pinnable against the REAL decision code (the W-C1-R rule: pin
// the seam, not a string-shape source check).
//
// stop = STOP_CHAIN/HOLD_CHAIN action present (the halting/pausing tick's own
// signal) OR the budget pause just landed (pausedNow) OR the COMMITTED state
// is held. The third arm is new in W-C3 and exists for ONE path: the pinger
// liveness MARKER commit (above) on an already-held chain — that tick commits
// (journal-only) with NO STOP/HOLD action (the chain was held BEFORE the
// tick), so the action-derived stop alone would self-dispatch and restart the
// self-tick loop at runner cadence — F2's livelock class, re-entered through
// the marker door. For every pre-W-C3 path the committed state's held flags
// coincide with a STOP/HOLD action (the halting tick mints STOP_CHAIN; the
// pause mints HOLD_CHAIN), so the third arm is truth-preserving there — belt
// AND suspenders, not a behavior change.
// ---------------------------------------------------------------------------
export function chainContinuationDecision({ actions = [], state = null, pausedNow = false } = {}) {
  const held = !!(state?.chain?.paused || state?.chain?.halted);
  return {
    stop: actions.some(a => a?.type === 'STOP_CHAIN' || a?.type === 'HOLD_CHAIN') || !!pausedNow || held,
    held,
  };
}
...
//
// T46/s21 C-2 (audit a1, MAJOR — the double-dispatch): the occupancy arm
// now lives in its own PRE-FLIGHT decision (overflowPreFlight) consulted
// BEFORE any wire attempt; workerOverflowDecision keeps ONLY the
// saturated-ladder fallback consulted AFTER a FAILED main attempt. The old
// shape decided the target AFTER the same-repo ladder had already 204'd,
// so `inFlightNow >= WORKER_OVERFLOW_AT` re-sent the SAME payload (same
// lease token) to BOTH buckets — two workers grinding one task (cross-repo
// supersession is impossible), 2x paid-key burn on cc epochs, the second
// report an orphan reject. Under the live WORKER_OVERFLOW_AT=1 this fired
// on every multi-assign tick.
// ---------------------------------------------------------------------------
...
// overflowPreFlight({ inFlightNow, repo2, pat, overflowAt }) — PURE, the
// occupancy arm. inFlightNow counts the leases outstanding BEFORE this
// dispatch (prior ticks' unreported leases + this turn's already-dispatched
// ones; the CURRENT task is not yet in flight — it is the dispatch being
// decided now). Fires -> the adapter sends THIS dispatch straight to repo2
// and SKIPS the same-repo attempt entirely (exactly one dispatch, one
// bucket). repo2/pat absent -> {overflow:false} for ANY input (the unset-var
// byte-identical contract).
export function overflowPreFlight({ inFlightNow = 0, repo2 = null, pat = null, overflowAt = WORKER_OVERFLOW_AT_DEFAULT } = {}) {
...
// workerOverflowDecision({ d, repo2, pat }) — PURE, the saturated-ladder
// FALLBACK arm: consulted only AFTER a same-repo attempt FAILED. d is the
// ladder's result ({ok:true} | {ok:false,...}); the decision fires on
// exactly one signal:
//   d.saturated — the ladder exhausted in the 403/429-with-Retry-After
//   class (the secondary-rate-limit shape). A bare-403 permission failure
//   stays fatal — re-targeting cannot fix a permission; a 5xx exhaustion
//   is not bucket pressure; and a SUCCESSFUL attempt (d.ok) NEVER
//   re-targets — that was the C-2 double-dispatch (one payload, one
//   bucket; if the main bucket took it, the wire work is done).
// repo2/pat absent -> {overflow:false} for ANY input (the unset-var
// byte-identical contract).
export function workerOverflowDecision({ d = null, repo2 = null, pat = null } = {}) {
  if (!repo2 || !pat) return { overflow: false };
