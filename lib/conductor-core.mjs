...
// [1,120] (intake.mjs LEASE_MIN/LEASE_MAX, bounds identical to
// CONFIG_BOUNDS.lease_minutes) and advertised verbatim in howToComment — is
// carried into the epoch's genesis config at the rollover and at
// reset {from_queue}. Previously validated-and-advertised but silently
// DROPPED: a mode:cc task with a 9-min grind on a chain configured with a
// short lease got lease-reaped mid-work, re-dispatched (duplicate spend),
// its late report orphaned 'stale-lease', and after 3 attempts the epoch
// ended DEGRADED — the user's completable work lost through a knob they
// were told works. PURE: returns the parsed int when the spec carries an
// in-bounds value (the door's YAML-ish parse keeps scalars as STRINGS —
// '45' — so parseInt here), else null (the chain's current config stands;
// out-of-bounds garbage is defense-in-depth — the door already rejects it).
...
export function conductorTick({
  ...
  if (intakeQueue.length && s.project.phase === 'done' && !s.chain.paused) {
    ...
    const cfg = { ...s.config, tick_min_interval_s: Math.max(s.config.tick_min_interval_s ?? 0, 25) };
    // s21/A4-F1: the spec's lease_minutes — validated [1,120] at the door,
