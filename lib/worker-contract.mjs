...
// s24/B1 (multi-engine design D1): 'codex' joins the vocabulary — the
// OpenAI Codex CLI engine rides the SAME envelope contract (mock | real |
// cc | codex). The adapter itself (worker/codex-adapter.mjs) ships in
// parallel as B2; until it lands the routing arm in worker/turn.mjs
// imports it lazily and reports infra_failed 'codex-adapter-missing'.
export const ENVELOPE_MODES = ['mock', 'real', 'cc', 'codex'];
...
// T46/W-C2: the declared-artifact pathspec cap riding the envelope (the
// write-back door's per-task cap is WRITE_BACK_CAP_TASK_BYTES; this bounds
// the COUNT so a hostile spec cannot mint an unbounded declaration list)
export const ENVELOPE_ARTIFACTS_MAX = 32;
...
export function envelopeFromDispatch(cp, nowMs = Date.now()) {
  ...
  // T46/W-C2 (§4/F-4): the DECLARED artifact pathspecs ride the envelope
  // (the task's spec.artifacts — the conductor's dispatch payload mints them
  // into ox). They become the write-back door's allowRoot at the worker: the
  // declared-artifact contract (root-level files are denied by default — a
  // declaration unlocks exactly its pathspec). Fail-closed on shape: a
  // corrupt declaration must never silently degrade into "no artifacts"
  // (that would flip a real write-back into an unvalidated one at the door's
  // default). Absent/empty → undefined (the §4c skip: no artifacts, no
  // branch, no PR).
  let artifacts;
  if (cp.artifacts !== undefined && cp.artifacts !== null) {
    if (!Array.isArray(cp.artifacts) || cp.artifacts.length === 0) {
      return fail('bad-artifacts', `cp.artifacts must be a non-empty array of pathspecs (got ${JSON.stringify(cp.artifacts)?.slice(0, 80)})`);
    }
    if (cp.artifacts.length > ENVELOPE_ARTIFACTS_MAX) {
      return fail('bad-artifacts', `cp.artifacts carries ${cp.artifacts.length} pathspecs (cap ${ENVELOPE_ARTIFACTS_MAX})`);
    }
    for (const a of cp.artifacts) {
      if (typeof a !== 'string' || a === '') {
        return fail('bad-artifacts', `cp.artifacts entries must be non-empty strings (got ${JSON.stringify(a)})`);
      }
      if (a.length > 256) {
        return fail('bad-artifacts', `cp.artifacts pathspec exceeds 256 chars (${a.slice(0, 60)}…)`);
      }
    }
  ...
  return {
    ...
    envelope: {
      ...
      mode,
      attempt,
      // T46/W-C2: present ONLY when the task declared artifacts (the §4c
      // skip rides absence, never an empty array)
      ...(artifacts ? { artifacts } : {}),
    },
  };
}
...
//      done + error-shaped marker content       -> infra_failed 'error-as-answer'
//                                                 (the E11 trust boundary is
//                                                 re-checked even when the
//                                                 harness vouches 'done' — but
//                                                 s21/W4: only in an error-line
//                                                 context, see the E11 GATE
//                                                 below; prose that merely
//                                                 MENTIONS a marker stays done)
...
//   4. error-shaped content matching a marker   -> infra_failed 'error-as-answer'
//                                                 (s21/W4: the E11 GATE — see
//                                                 below)
...
//
// THE E11 GATE (s21/W4, a2): a marker SUBSTRING alone no longer convicts.
// The E11 class is error TEXT packaged AS the answer — not an answer that
// merely DISCUSSES an error. The hit must sit in an error-line context:
//   (a) the content opens with a generic error prefix — "Error:",
//       "API Error:", "FATAL:" — or a bare HTTP status ("401 Unauthorized — …"):
//       the upstream passthrough shapes (X21-final verbatim: "API Error:
//       Request rejected (429) · Rate limit exceeded: …"); or
//   (b) the marker MATCH opens the content ("unauthorized", "RATE LIMIT
//       exceeded, slow down", "Insufficient credits: add more") — the
//       bare/marker-headed answer.
// Pre-W4 ANY non-empty content CONTAINING a marker was infra — a
// legitimately DONE turn whose result mentioned "rate limit" (this repo's
// own task mix: runbooks for the 429 lane, quota docs) flipped to infra →
// net-zero ×3 → infra-exhausted QUARANTINE of good, landed work.
...
// the error-line openers for gate (a): the shapes upstream errors arrive in
const ERROR_LINE_RE = /^\s*(?:(?:api\s+)?error|fatal)\b/i;
const STATUS_LINE_RE = /^\s*\d{3}\b/;
...
export function classifyOutcome(raw, ctx = {}) {
  ...
  // s21/W4 (a2): matchMarkerIndex returns WHERE the marker hits (-1 when
  // absent) so the E11 GATE can require an error-line context (see the
  // E11 GATE note above the exports)
  const matchMarkerIndex = (text, marker) => {
    if (typeof marker === 'string') return text.toLowerCase().indexOf(marker.toLowerCase());
    if (marker instanceof RegExp) {
      // a /g regex is STATEFUL across .exec() calls — strip the flag so the
  ...
  const markerHit = (text) => {
    const head = text.length - text.trimStart().length;   // where the content actually opens
    const errorLine = ERROR_LINE_RE.test(text) || STATUS_LINE_RE.test(text);
    for (const mk of markers) {
      const at = matchMarkerIndex(text, mk);
      if (at >= 0 && (errorLine || at === head)) return markerName(mk);
    }
    return null;
  };
  ...
  // 4. error-as-answer (E11): error TEXT packaged as the success answer —
  // checked BEFORE done. s21/W4: the E11 GATE (markerHit) requires an
  // error-line context, so prose that merely MENTIONS a marker falls
  // through to done
  if (hasContent) {
    const hit = markerHit(raw.content);
    if (hit) return { status: 'infra_failed', detail: `error-as-answer(${hit})` };
