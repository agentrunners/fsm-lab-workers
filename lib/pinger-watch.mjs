...
// (default 3h ≈ 3 missed heartbeats) → the watchdog itself has stopped
...
// The deadman window: the marker heartbeat is ~1/hour (throttled), so
// 300min ≈ FIVE missed heartbeats. s22/R2-1 recalibration (the A-1 logic
// applied to the deadman): the watchdog's own sibling-run gaps run
// 2h00m-4h07m LIVE (the same measured cadence the pinger window was
// recalibrated on) — the s21 180min default false-alarmed a healthy chain
// whose scan gap landed at 3h. 300 sits above the measured 4h07 max with
// ~1h headroom (one missed marker inside 300min still reads fresh; a dead
// watchdog is still caught same-day). WATCHDOG_STALE_AFTER_MIN env
// overrides it at the duty host (watchdogStaleAfterMin). The executor-side
// roster carries the same 300 (watchdog_stale_after_min — the roster is the
// driver's source of truth; the executor never hardcodes it).
