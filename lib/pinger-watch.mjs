...
// (default 5h — s22/R2-1 recalibrated, see below) → the watchdog itself has stopped
...
// The deadman window: nominal marker cadence ~1/hour (WATCHDOG_MARKER_MIN,
// default 55), so the ORIGINAL 180min ≈ three missed hourly heartbeats looked
// safe — but that arithmetic is calibrated to the NOMINAL cadence, the exact
// mistake A-1 caught on the pinger one commit earlier (R2-1, the s21 lens-2
// review): GHA schedule sparsity is live-proven for this repo's cron class
// (the pinger's nominal */15 delivers MEASURED 2h00m–4h07m gaps), and a
// sparsed watchdog scan stream throttles to nothing — the marker fires on
// the first scan after 55min, so marker gaps ≈ scan gaps ≈ the measured
// multi-hour band, and 180min fires "watchdog SILENT" on healthy markers
// (the A-1 false-alarm-noise class, repeated on the deadman). s22/R2-1
// recalibrates to 300min — the SAME headroom rule as the pinger window:
// above the measured max (~4h07) with ~1h headroom, still same-day
// detection; stress/batteries/soak30d.mjs pins the before/after numbers
// (0 false alarms at 300min over 30 virtual days at the measured cadence vs
// N>0 at the old 180min). WATCHDOG_STALE_AFTER_MIN env overrides it at the
// duty host (watchdogStaleAfterMin).
