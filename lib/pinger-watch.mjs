...
// The deadman window: the marker heartbeat is ~1/hour, so 180min ≈ THREE
// missed heartbeats — the balance between fast detection and tolerance of
// one missed marker (GHA schedule sparsity is real; two in a row inside
// 180min still reads fresh). WATCHDOG_STALE_AFTER_MIN env overrides it at
// the duty host (watchdogStaleAfterMin).
export const WATCHDOG_STALE_AFTER_MIN = 180;
