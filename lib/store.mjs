...
export class Store {
  ...
  // W-D review fold (A4, lens-2 F2): the OPTIONAL `kind` filter — when set
  // (e.g. 'REPORT'), the scan skips non-matching records and `n` bounds the
  // MATCHED count (the console's journal-sourced LANE section needs the last
  // N REPORT records, not the last N records of any kind). Unset = today's
  ...
  readJournalTail(n = 64, kind = null) {
    ...
    for (const g of gens.reverse()) {
      ...
      for (const line of lines.reverse()) {
        let rec;
        try { rec = JSON.parse(line); } catch { console.warn(`journal-${g}: unparseable line skipped (tail read)`); continue; }
